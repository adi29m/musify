...
export function ArtistClient({ tracks }: { tracks: UiTrack[] }) {
  const { playTracks } = usePlayer();
  if (tracks.length === 0) return <p className="text-sm text-zinc-500">No playable tracks for this artist right now.</p>;
  return (
    <div>
      <button onClick={() => playTracks(tracks, 0)} className="mb-4 flex items-center gap-2 rounded-full bg-green-500 px-6 py-3 text-sm font-bold text-black hover:scale-105">
        <Play size={16} fill="currentColor" /> Play artist
      </button>
